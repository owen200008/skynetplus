...
int32_t CCoroutineCtx_NetClientServerCommu::OnDisconnect(basiclib::CBasicSessionNetClient* pNotify, Net_UInt dwNetCode){
    ctx_message ctxMsg(0, CCoroutineCtx_NetClientServerCommu::Func_ReceiveMysqlDisconnect);
    PushMessage(ctxMsg);
    return BASIC_NET_OK;
}

void CCoroutineCtx_NetClientServerCommu::Func_ReceiveMysqlDisconnect(CCoroutineCtx* pCtx, ctx_message* pMsg, CCtx_CorutinePlusThreadData* pData){
    CCoroutineCtx_NetClientServerCommu* pMySql = (CCoroutineCtx_NetClientServerCommu*)pCtx;
    pMySql->ReceiveDisconnect(pMsg, pData);
}
void CCoroutineCtx_NetClientServerCommu::ReceiveDisconnect(ctx_message* pMsg, CCtx_CorutinePlusThreadData* pData){
    //断开
...
void CCoroutineCtx_NetClientServerCommu::OnTimerCheckTimeout(CCoroutineCtx* pCtx, CCtx_CorutinePlusThreadData* pData){
    ...
    IsErrorHapper(pRequestStore == nullptr || !pRequestStore->IsTimeOut(tmNow, pRealCtx->m_nDefaultTimeoutRequest), CCFrameSCBasicLogEventErrorV(nullptr, "%s(%s:%d) vt Timeout, Close", __FUNCTION__, __FILE__, __LINE__); pRealCtx->ErrorClose(); return);
    for (auto& data : pRealCtx->m_mapRequest){
        IsErrorHapper(!data.second.IsTimeOut(tmNow, pRealCtx->m_nDefaultTimeoutRequest), CCFrameSCBasicLogEventErrorV(nullptr, "%s(%s:%d) map Timeout, Close", __FUNCTION__, __FILE__, __LINE__); pRealCtx->ErrorClose(); return);
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//! 协程里面调用Bussiness消息
int CCoroutineCtx_NetClientServerCommu::DispathBussinessMsg(CCorutinePlus* pCorutine, CCtx_CorutinePlusThreadData* pData, uint32_t nType, int nParam, void** pParam, void* pRetPacket, ctx_message* pCurrentMsg){
    switch (nType){
    case DEFINEDISPATCH_CORUTINE_NetClient_CreateMap:
        return DispathBussinessMsg_CreateMap(pCorutine, pData, nParam, pParam, pRetPacket);
    case DEFINEDISPATCH_CORUTINE_NetClient_DeleteMap:
        return DispathBussinessMsg_DelMap(pCorutine, pData, nParam, pParam, pRetPacket);
    case DEFINEDISPATCH_CORUTINE_NetClient_GetMap:
        return DispathBussinessMsg_GetMap(pCorutine, pData, nParam, pParam, pRetPacket);
    case DEFINEDISPATCH_CORUTINE_NetClient_Request:
        return DispathBussinessMsg_Request(pCorutine, pData, nParam, pParam, pRetPacket);
    default:
        return CCoroutineCtx_NetClient::DispathBussinessMsg(pCorutine, pData, nType, nParam, pParam, pRetPacket, pCurrentMsg);
    }
    return -99;
}
...
long CCoroutineCtx_NetClientServerCommu::DispathBussinessMsg_Receive(CCorutinePlus* pCorutine, CCtx_CorutinePlusThreadData* pData, int nParam, void** pParam, void* pRetPacket, ctx_message* pCurrentMsg){
    ...
    CCorutineStackDataDefault* pStack = (CCorutineStackDataDefault*)pParam[0];
    int nStackDataLength = 0;
    const char* pStackData = pStack->GetData(nStackDataLength);
    Net_UInt nMethod = 0;
    ...
    if (bResponse){
        ...
        case SPROTO_METHOD_SKYNET_Register:{
            ...
            IsErrorHapper(InsToSprotoStruct(response, (char*)pStackData, nStackDataLength), ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) parse register error, Close", __FUNCTION__, __FILE__, __LINE__); ErrorClose(); return 0);
            if (response.m_nError != 0){
                ASSERT(0);
                CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) register ret error(%d), Close", __FUNCTION__, __FILE__, __LINE__, response.m_nError);
                ErrorClose();
            }
            else{
                CCFrameSCBasicLogEvent(pData, "Register GlobalServer Success!");
                //重新发送所有的注册
                if (m_map.size() > 0){
        ...
        case SPROTO_METHOD_SKYNET_CreateNameToCtxID:{
            ...
            IsErrorHapper(InsToSprotoStruct(response, (char*)pStackData, nStackDataLength), ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) parse createnametoctxid error, Close", __FUNCTION__, __FILE__, __LINE__); return 0);
            if (response.m_nError != 0){
                ASSERT(0);
                CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) createNameToCtxid ret error(%d), Close", __FUNCTION__, __FILE__, __LINE__, response.m_nError);
                ErrorClose();
            }
        }
                                                    break;
        case SPROTO_METHOD_SKYNET_DeleteNameToCtxID:{
            ...
            IsErrorHapper(InsToSprotoStruct(response, (char*)pStackData, nStackDataLength), ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) parse delnametoctxid error", __FUNCTION__, __FILE__, __LINE__); return 0);
            if (response.m_nError != 0){
                ASSERT(0);
                CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) deleteNameToCtxid ret error(%d), Close", __FUNCTION__, __FILE__, __LINE__, response.m_nError);
                ErrorClose();
            }
        }
                                                    break;
        case SPROTO_METHOD_SKYNET_GetCtxIDByName:{
            ...
            IsErrorHapper(InsToSprotoStruct(response, (char*)pStackData, nStackDataLength), ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) parse delnametoctxid error, Close", __FUNCTION__, __FILE__, __LINE__); ErrorClose(); return 0);
            //notify
            KernelRequestStoreData* pRequest = m_vtRequest.FrontData();
            IsErrorHapper(nullptr != pRequest, ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) NoRequest Error, Close", __FUNCTION__, __FILE__, __LINE__); ErrorClose(); break);
            ctx_message sendCtxMsg;
            sendCtxMsg.m_session = (intptr_t)&response;
            IsErrorHapper(WaitResumeCoroutineCtx(pRequest->m_pCorutine, this, pData, &sendCtxMsg), ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) WaitResumeCoroutineCtx Fail,Close", __FUNCTION__, __FILE__, __LINE__); ErrorClose(); break);
        }
                                                 break;
        case SPROTO_METHOD_SKYNETCHILD_Request:{
            ...
            response.m_pIns = &ins;
            IsErrorHapper2(InsToSprotoStruct(response, (char*)pStackData, nStackDataLength), 
                ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) parse delnametoctxid error", __FUNCTION__, __FILE__, __LINE__),
                return 0);
            auto& iter = m_mapRequest.find(response.m_head.m_nSession);
            if (iter != m_mapRequest.end()){
                ctx_message sendCtxMsg;
                sendCtxMsg.m_session = (intptr_t)&response;
                IsErrorHapper(WaitResumeCoroutineCtx(iter->second.m_pCorutine, this, pData, &sendCtxMsg), ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) WaitResumeCoroutineCtx Fail,Close", __FUNCTION__, __FILE__, __LINE__); ErrorClose(); break);
            }
            else{
                CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) no sessionid find %d", __FUNCTION__, __FILE__, __LINE__, response.m_head.m_nSession);
            }
        }
                                               break;
        default:
        ...
        case SPROTO_METHOD_SKYNETCHILD_ServerRequest:{
            ...
            IsErrorHapper2(InsToSprotoStruct(request, (char*)pStackData, nStackDataLength),
                ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) parse ServerRequest error", __FUNCTION__, __FILE__, __LINE__),
                return 0);
            //拷贝session,ctxid
            ...
            do{
                ...
                IsErrorHapper2(InsToSprotoStruct(realRequest, childRequest.GetDataBuffer(), childRequest.GetDataLength()),
                    ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) parsechild ServerRequest error", __FUNCTION__, __FILE__, __LINE__),
                ...
                IsErrorHapper2(CCoroutineCtxHandle::GetInstance()->IsRemoteServerCtxID(realRequest.m_request.m_nDstCtxID) == false,
                    ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) receive remotectx(%x) type(%d) error", __FUNCTION__, __FILE__, __LINE__, realRequest.m_request.m_nDstCtxID, realRequest.m_request.m_nType),
                ...
                IsErrorHapper2(iterCtxMap != m_mapRegister.end(),
                    ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) receive no serialize remotectx(%x) type(%d) error", __FUNCTION__, __FILE__, __LINE__, realRequest.m_request.m_nDstCtxID, realRequest.m_request.m_nType),
                ...
                bool bRetUnSerializeRequest = funcData.m_requestUnSerialize(realRequest.m_ins, [&](int nParam, void** pParam)->bool{
                    IsErrorHapper2(nParam == realRequest.m_cParamCount,
                        ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) param count error remotectx(%x) type(%d) %d != %d", __FUNCTION__, __FILE__, __LINE__, realRequest.m_request.m_nDstCtxID, realRequest.m_request.m_nType, nParam, realRequest.m_cParamCount),
                        return false);
                    //去本机ctxID
                    ...
                    bool bRetSerializeResponse = funcData.m_responseSerialize([&](void* pRetPacket)->bool{
                        IsErrorHapper2(YieldCorutineToCtx(pCorutine, nDstCtxID, pCurrentCtx, pCurrentData, pCurrentMsg),
                            ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) YieldCorutineToCtx error remotectx(%x) type(%d)", __FUNCTION__, __FILE__, __LINE__, realRequest.m_request.m_nDstCtxID, realRequest.m_request.m_nType),
                            return false);
                        nRetValue = pCurrentCtx->DispathBussinessMsg(pCorutine, (CCtx_CorutinePlusThreadData*)pCurrentData, realRequest.m_request.m_nType, nParam, pParam, pRetPacket, pCurrentMsg);
                    ...
                    IsErrorHapper2(nRetValue == 0 && bRetSerializeResponse,
                        ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) DispathBussinessMsg error remotectx(%x) type(%d)", __FUNCTION__, __FILE__, __LINE__, realRequest.m_request.m_nDstCtxID, realRequest.m_request.m_nType),
                        return false);
                    return true;
                });
                IsErrorHapper2(bRetUnSerializeRequest,
                    ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) m_requestUnSerialize error remotectx(%x) type(%d)", __FUNCTION__, __FILE__, __LINE__, realRequest.m_request.m_nDstCtxID, realRequest.m_request.m_nType),
...
long CCoroutineCtx_NetClientServerCommu::DispathBussinessMsg_CreateMap(CCorutinePlus* pCorutine, CCtx_CorutinePlusThreadData* pData, int nParam, void** pParam, void* pRetPacket){
...
long CCoroutineCtx_NetClientServerCommu::DispathBussinessMsg_DelMap(CCorutinePlus* pCorutine, CCtx_CorutinePlusThreadData* pData, int nParam, void** pParam, void* pRetPacket){
    IsErrorHapper(nParam == 1, ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) paramerror(%d)", __FUNCTION__, __FILE__, __LINE__, nParam); return -1);
    if (!IsTransmit())
        return -2;
    const char* pCtxName = (const char*)pParam[0];
...
long CCoroutineCtx_NetClientServerCommu::DispathBussinessMsg_GetMap(CCorutinePlus* pCorutine, CCtx_CorutinePlusThreadData* pData, int nParam, void** pParam, void* pRetPacket){
    IsErrorHapper(nParam == 1, ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) paramerror(%d)", __FUNCTION__, __FILE__, __LINE__, nParam); return -1);
    const char* pCtxName = (const char*)pParam[0];
    if (!IsTransmit())
        return -2;
...
long CCoroutineCtx_NetClientServerCommu::DispathBussinessMsg_Request(CCorutinePlus* pCorutine, CCtx_CorutinePlusThreadData* pData, int nParam, void** pParam, void* pRetPacket){
    IsErrorHapper(nParam == 4, ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) paramerror(%d)", __FUNCTION__, __FILE__, __LINE__, nParam); return -1);
    if (!IsTransmit())
        return -2;
    ...
    IsErrorHapper(funcData.m_requestSerialize(nRequestParam, pRequestParam, request.m_ins), ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) m_requestSerialize error ctxid(%x) type(%d)", __FUNCTION__, __FILE__, __LINE__, nDstCtxID, nType); return 3);
    //! 保存
    ...
    IsErrorHapper(funcData.m_responseUnSerialize(*pResponse->m_pIns, pRetPacket), 
        ASSERT(0); CCFrameSCBasicLogEventErrorV(pData, "%s(%s:%d) m_responseUnSerialize error ctxid(%x) type(%d)", __FUNCTION__, __FILE__, __LINE__, nDstCtxID, nType); return 5);
    return 0;
}
